<template>
  <div class="bills-page">
    ...
    <MacDialog v-model="showBillDialog" :title="billDialogMode === 'create' ? '快速记一笔' : '编辑账单'" width="720px" :close-disabled="submitting" panel-class="personal-bill-dialog" @cancel="closeBillDialog">
    ...
    <MacDialog v-model="showBudgetDialog" :title="budgetDialogMode === 'create' ? '新增年度预算' : '编辑年度预算'" width="620px" :close-disabled="budgetSubmitting" panel-class="personal-budget-dialog" @cancel="showBudgetDialog = false">
      <form id="personal-budget-dialog-form" class="budget-form" @submit.prevent="submitBudgetDialog">
...
.bills-page{--ink:#eaf6ff;--muted:#8ea7ba;--line:rgba(124,190,224,.14);--blue:#55a7ff;--cyan:#53d6d0;background:radial-gradient(circle at 12% 0,rgba(22,100,153,.25),transparent 34%),radial-gradient(circle at 92% 18%,rgba(16,120,118,.14),transparent 28%),linear-gradient(145deg,#061722,#0a2230 48%,#071a26);color:var(--ink)}
.topbar h1,.section-heading h2,.day-group>header strong,.bill-copy b,.bill-meta b,.budget-list article button{color:#edf8ff}.eyebrow,.section-heading span{color:#6bc5e8}.icon-button,.soft-button{background:rgba(9,36,52,.78);color:#dff5ff;border:1px solid rgba(122,196,230,.18);box-shadow:0 10px 28px rgba(0,0,0,.2)}.month-hero{border-color:rgba(101,191,224,.23);background:linear-gradient(120deg,#0a2739,#103d58 55%,#0d5961);box-shadow:0 24px 58px rgba(0,0,0,.3)}
.view-tabs button{color:#7794a8}.view-tabs button.active{background:rgba(16,54,73,.86);color:#72c8ff;box-shadow:0 9px 24px rgba(0,0,0,.2)}.ledger-panel,.insight-card{border-color:rgba(111,184,218,.16);background:linear-gradient(145deg,rgba(12,42,58,.92),rgba(7,29,42,.9));box-shadow:0 18px 48px rgba(0,0,0,.24)}

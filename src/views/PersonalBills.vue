  ...
  <div class="bills-page">
      ...
      <aside class="insight-panel" :class="{ 'mobile-hidden': activeView !== 'stats' }">
        ...
        <section class="insight-card budget-card-section">
          ...
          <div v-if="currentYearBudgets.length" class="budget-list">
            <article v-for="item in currentYearBudgets" :key="item.categoryName">
              <div class="budget-item-head"><span><b>{{ item.categoryName }}</b><small> · {{ item.statusText }}</small></span><strong>{{ formatCurrency(item.usedAmount) }} / {{ formatCurrency(item.annualLimit) }}</strong></div>
              <div><i :class="item.statusClass" :style="{ width: `${Math.min(item.usageRate, 100)}%` }"></i></div>
              <section v-for="detail in budgetDetails.filter(detail => detail.categoryName === item.categoryName)" :key="detail.id" class="budget-detail-row">
                <span>{{ detail.budgetName || detail.categoryName }}<small v-if="detail.note"> · {{ detail.note }}</small></span>
                <strong>{{ formatCurrency(detail.annualLimit) }}</strong>
                <button class="text-button" @click="openEditBudgetDialog(detail)">编辑</button>
                <button class="text-button" :disabled="budgetSubmitting" @click="removeBudget(detail)">删除</button>
              </section>
            </article>
          </div>
    ...
    <MacDialog v-model="showBillDialog" :form-state="{...billForm, amount: amountExpression}" :form-baseline-key="billBaselineKey" :title="billDialogMode === 'create' ? '快速记一笔' : '编辑账单'" width="860px" :close-disabled="submitting" panel-class="personal-bill-dialog" @cancel="closeBillDialog">
      <form id="personal-bill-dialog-form" class="entry-form" @submit.prevent="submitBillDialog(false)">
        ...
        <p class="field-help">渠道记录如何收付款，资金账户记录钱从哪里出入；例如微信支付 + 招行信用卡。还款和本人账户互转不作为消费或收入重复记录。</p>
        <div class="detail-fields">
          <label><span>日期</span><input v-model="billForm.billDate" type="date" required /></label>
          <label><span>资金账户</span><input v-model.trim="billForm.accountName" maxlength="32" placeholder="例如：招行信用卡尾号 1234 / 微信零钱" /></label>
          <label><span>收付款渠道</span><select v-model="billForm.paymentMethod"><option value="">未填写</option><option v-for="item in paymentMethodOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
          <label><span>交易对方</span><input v-model.trim="billForm.merchantName" maxlength="48" placeholder="消费地点或收入来源" /></label>
    ...
    <MacDialog v-model="showBudgetDialog" :form-state="budgetForm" :title="budgetDialogMode === 'create' ? '新增年度预算' : '编辑年度预算'" width="860px" :close-disabled="budgetSubmitting" panel-class="personal-budget-dialog" @cancel="showBudgetDialog = false">
      <form id="personal-budget-dialog-form" class="budget-form dialog-density-grid dialog-grid-cols-4" @submit.prevent="submitBudgetDialog">
        ...
        <label><span>预算项目</span><input v-model.trim="budgetForm.budgetName" maxlength="64" placeholder="例如：奶粉 / 托育 / 兴趣班" required /></label>
        <label><span>项目年度额度</span><input v-model.number="budgetForm.annualLimit" type="number" min="0.01" step="0.01" required /></label>
        <label><span>预警比例（0.8 表示 80%）</span><input v-model.number="budgetForm.alertThreshold" type="number" min="0.01" max="1" step="0.01" required /></label>
...
import PersonalCreditPanel from '@/components/personal-bills/PersonalCreditPanel.vue'
...
const budgetForm = reactive({ year: new Date().getFullYear(), categoryName: '', budgetName: '', annualLimit: 0, alertThreshold: 0.8, note: '' })
...
// 完整校验表达式后再计算，避免粘贴非法内容时静默截取部分金额。
const normalizedAmountExpression = computed(() => amountExpression.value.replace(/-/g, '−').replace(/\s/g, ''))
const calculatedAmount = computed(() => {
  const expression = normalizedAmountExpression.value
  if (!expression) return '0.00'
  if (!/^(?:\d+(?:\.\d*)?|\.\d+)(?:[+−](?:\d+(?:\.\d*)?|\.\d+))*$/.test(expression)) return '请输入完整金额'
  const parts = expression.match(/(?:\d+(?:\.\d*)?|\.\d+)|[+−]/g)
  let value = Number(parts[0])
  for (let index = 1; index < parts.length; index += 2) value += (parts[index] === '+' ? 1 : -1) * Number(parts[index + 1])
  return Number.isFinite(value) ? value.toFixed(2) : '金额过大'
})
...
async function loadBills() {
  ...
  try {
    ...
    budgetDetails.value = unwrap(budgetsResponse) || []
...
function handleKeypad(key) {
  ...
  amountExpression.value = normalizedAmountExpression.value
...
async function submitBillDialog(keepOpen) {
  ...
  if (!Number.isFinite(billForm.amount)) return window.alert('请输入有效金额或完整的加减算式')
...
function openCreateBudgetDialog() { budgetDialogMode.value = 'create'; editingBudget.value = null; Object.assign(budgetForm, { year: selectedYear.value, categoryName: budgetCategoryOptions.value[0]?.value || '', budgetName: '', annualLimit: 0, alertThreshold: 0.8, note: '' }); showBudgetDialog.value = true }
...
function openEditBudgetDialog(item) { budgetDialogMode.value = 'edit'; editingBudget.value = item; Object.assign(budgetForm, { year: item.year || selectedYear.value, categoryName: item.categoryName, budgetName: item.budgetName || item.categoryName, annualLimit: item.annualLimit, alertThreshold: item.alertThreshold, note: item.note || '' }); showBudgetDialog.value = true }
